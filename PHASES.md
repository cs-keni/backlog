...
- [x] One-off DB cleanup (`scripts/cleanup-phantom-applications.mjs`) — investigated all 29 `hide_from_feed: true` stub records live; found the situation was more nuanced than "27 obvious junk rows" (several were genuine applications — TikTok progressed to "technical" stage, 6 separate Mastercard submissions sharing one generic confirmation title — that must NOT be deleted). Categorized into junk vs. duplicate-clusters vs. real-but-badly-titled, presented findings to the user, and ran the script after confirmation. Deleted 14 phantom `jobs`+`applications` pairs: 5 unambiguous junk stubs (4× the user's own GitHub pages, 1× a Wells Fargo talent-community signup) and 9 duplicate stubs collapsed down to one record each for two real multi-step applications (HR Block job 42434: kept `b78e19ba-...`, deleted 6 step-by-step dupes; GDMS job 72592: kept `e3a23411-...`, deleted 3 step-by-step dupes). Left one ambiguous standalone stub (GDMS job 72580, "Enter Your Information") untouched — no confirmation record exists for it, so it can't be confidently classified as junk vs. an in-progress application.
